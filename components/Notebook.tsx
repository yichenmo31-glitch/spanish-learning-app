...
export const Notebook: React.FC<NotebookProps> = ({ words }) => {
  ...
  if (reviewing && words.length > 0) {
    return (
      <div className="max-w-5xl mx-auto py-20 px-8 animate-in fade-in duration-500">
        <Flashcards words={words} onExit={() => setReviewing(false)} />
      </div>
    );
  }
        ...
        <div className="flex items-center gap-3">
          {words.length > 0 && (
            <button
              ...
              className="px-5 py-2.5 bg-slate-900 text-white rounded-full text-xs font-bold uppercase tracking-widest hover:bg-slate-800 transition-all active:scale-[0.98]"
            >
              Review ({words.length})
            </button>
          )}
          <div className="px-5 py-2.5 bg-slate-100 text-slate-600 rounded-full text-xs font-bold uppercase tracking-widest">
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                {isSpeechSupported() && (
                  <button
                    ...
                    className="p-2 bg-slate-50 text-slate-400 rounded-xl hover:bg-slate-100 hover:text-slate-900 transition-colors"
                  >
